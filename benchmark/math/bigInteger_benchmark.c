 ...
 * n =  10
 */
static void init_e(bigInteger *state) {
  bigInteger_set_int(state    , 9864101);
  bigInteger_set_int(state + 1, 3628800);
  bigInteger_set_int(state + 2, 1);
  bigInteger_set_int(state + 3, 10);
}
static char extract_e(bigInteger *state) {
 ...
 * m =  56
 */
static void init_phi(bigInteger *state) {
...
static char extract_phi(bigInteger *state) {
  // phi term 
  do {
 ...
 * m =  48
 */
static void init_sqrt3(bigInteger *state) {
...
static char extract_sqrt3(bigInteger *state) {
 ...
 * m =  32
 */
static void init_sqrt2(bigInteger *state) {
...
static char extract_sqrt2(bigInteger *state) {
 ...
 *
 */
static void init_pi(bigInteger *state) {
...
static char extract_pi(bigInteger *state) {
...
int main (int UNUSED_ARG(argc), char **UNUSED_ARG(argv)) {
  ...
  bigInteger_reserve(state + 0, 2805);
  bigInteger_reserve(state + 1, 2576);
  bigInteger_reserve(state + 2, 2576);
  bigInteger_reserve(state + 3,   10);
  bigInteger_reserve(state + 4, 2789);
  bigInteger_reserve(state + 5, 2789);
...
#define MAX_DIGITS 2*1024
  ...
  dstring_free(&qstr);
	for (i = 0; i < CACHE; ++i) {
    PRINT_INF("bigInteger %zu capacity: %hu\n", i, state[i].cap);
	  cnt += state[i].cap;
	  bigInteger_free(state + i);
	}
	{
