...
void *piPartWorker(void *arg)
{
	...
	pi_part = static_cast<uint32_t *>(calloc(gWordPrec, sizeof(uint32_t)));
	while (1)
	{
...
void *printPiWorker(void *arg)
{
	ProdPipeline *pp = static_cast<ProdPipeline *>(arg);
	uint32_t overflow;
	uint64_t sum;
...
int main(int argc, char *argv[])
{
	pthread_t *threads;
	ProdPipeline *pp;
	timeval tm_beg, tm_end, tm_diff;
	unsigned i;
	//number should 1 - 504000 may be more;
	gDigPrec = (5000) * 10; // this the maximum number that posible
	// 104/1000 ~= 1/log(2^32)
	...
	unsigned int threadCount = 1;// sysconf(_SC_NPROCESSORS_ONLN);
	if (threadCount >= gWordPrec)
		threadCount = gWordPrec - 1;
	pp = static_cast<ProdPipeline *>(malloc(threadCount * sizeof(ProdPipeline)));
	pi = static_cast<uint32_t *>(calloc(gWordPrec, sizeof(uint32_t)));
	threads = static_cast<pthread_t *>(malloc((threadCount - 1) * sizeof(pthread_t)));
