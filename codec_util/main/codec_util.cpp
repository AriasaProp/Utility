...
codec_data &operator<< (codec_data &o, unsigned long in) {
  ...
  o.check_resize (o.used_byte + (o.used_bit > 0));
  if (o.used_bit) {
...
std::ostream &operator<< (std::ostream &c, const codec_data &d) {
  ...
  if (d.used_bit) {
    c << std::setw (2) << std::setfill ('0') << std::hex << static_cast<int> (*end_ & ((0x1 << d.used_bit) - 1));
  }
  while ((--end_) >= begin_)
    c << std::setw (2) << std::setfill ('0') << std::hex << static_cast<int> (*end_ & 0xff);
  c << std::dec;
  return c;
}
