...
codec_data::reader &operator>> (codec_data::reader &o, unsigned char &d) {
  if (o.left () >= CHAR_BIT) {
    ...
    ++o.readed_byte;
    std::cout << "Read: " << int ((*dt >> o.readed_bit) | (o.readed_bit ? *(dt + 1) << (CHAR_BIT - o.readed_bit) : 0)) << std::endl;
  }
  return o;
}
...
codec_data &operator<< (codec_data &o, unsigned char in) {
  ...
  *dt |= in << o.used_bit;
  ...
  ++o.used_byte;
  std::cout << "Write: " << int ((*dt >> o.used_bit) | (o.used_bit ? *(dt + 1) << (CHAR_BIT - o.used_bit) : 0)) << std::endl;
  return o;
}
