...
bool matrix4_test() {
	...
	matrix4 a, b, c, d;
	{
		a = (float[]){6,9,7,2, 6,1,8,21, 6,1,12,4, 7,12,13,9};
		b = (float[]){9,9,2,1, 8,6,2,1, 8,6,5,9, 6,4,7,24, 7,9,4,9};
		c = (float[]){15,18,9,3, 14,7,13,30, 12,5,19,28, 14,21,17,18};
	}
	// "+" operator test
	result = true;
	d = a + b;
	...
	if (!result) std::cout << "    '-=' operator error \n";
	// "*" operator test
	c = (float[]){182,154,114,273, 257,281,157,396, 162,144,117,339, 300,268,201,508};
	d = a * b;
	result &= c == d;
	passed &= result;
	if (!result) std::cout << "    '*' operator error \n";
