...
int compare(const std::vector < word > a,
...
void add_word(std::vector < word > & a,
  const std::vector < word > & b) {
...
// a should clone from the base, b not
void sub_word(std::vector < word > & a,
  const std::vector < word > & b) {
  const size_t j = b.size();
  if (!j)
    return;
...
bool BigInteger::operator == (const BigInteger & b) const {
  if (neg != b.neg)
    return false;
  return compare(words, b.words) == 0;
}
bool BigInteger::operator != (const BigInteger & b) const {
  if (neg != b.neg)
    return true;
...
bool BigInteger::operator <= (const BigInteger & b) const {
  if (neg != b.neg)
    return neg;
  return 0 >= (compare(words, b.words) * (neg ? -1 : +1));
}
bool BigInteger::operator >= (const BigInteger & b) const {
  if (neg != b.neg)
    return b.neg;
